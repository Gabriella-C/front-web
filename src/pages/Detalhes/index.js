...
function Detalhes() {
  ...
  useEffect(() => {
    ...
    axios.post('http://localhost:3333/List_Pedidos', { 'id': id })
      .then(
        response => {
          setDados_usuario(response.data);
        }
      );
    axios.post('http://localhost:3333/Order_Details', { 'id': location.state.id })
      .then(
        response => {
  ...
  function finalizar() {
    if (!marked4) {
      alert('Esse pedido ainda não foi entregue, portanto não pode finaliza-lo!')
    } else {
      axios.post('http://localhost:3333/Order_Status_Update', { 'id': idpedido })
        .then(response => {
  ...
  return (
    <Container>
      <Content>
        {dados_usuario.map(({ idpedido, nome_usuario, celular, celular2, cep, endereco, bairro, cidade, estado, numero, complemento, data_pedido, data_previsao, hora_previsao }) => (
          <>
            <div>
            ...
            <div>
              ...
              <p> Número de telefone: {celular} | {celular2}</p>
            </div>
        ...
        <div>
          ...
          <div>
            <p>Aceitar Pedido</p>
            <Marked isMarked={marked} onClick={() => orderStatus('Aceitar Pedido')} />
          </div>
          <div>
            <p>Em Separação</p>
            <Marked isMarked={marked2} onClick={() => orderStatus('Em Separação')} />
          </div>
          <div>
            <p>Saiu para entrega</p>
            <Marked isMarked={marked3} onClick={() => orderStatus('Saiu para entrega')} />
          </div>
          <div>
            <p>Entregue</p>
            <Marked isMarked={marked4} onClick={() => orderStatus('Entregue')} />
          </div>
        </div>
